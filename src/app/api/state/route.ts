...
export async function GET() {
  ...
  const notificationPrefs = parseNotificationPrefs(appState?.notificationPrefs);

  return NextResponse.json(
    { conversationsByUrn, lastSyncedAt, myProfileUrn, profileName, profileAvatarUrl, notificationPrefs },
    { headers: CORS },
  );
}
